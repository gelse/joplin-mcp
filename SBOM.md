...
The integration-test stack (`docker-compose.test.yml` / `make test-integration`) uses the same `Dockerfile.combined` as production.
...
> **Note:** The integration-test stack uses the same `Dockerfile.combined` as production. See [`docker-compose.test.yml`](docker-compose.test.yml).
...
| `joplin` (CLI) | 3.7.1   | Joplin CLI — core data access layer; all API calls go through the `joplin` command |
...
The integration-test stack uses the same `Dockerfile.combined` and `entrypoint-combined.sh` as production. See [`docker-compose.test.yml`](docker-compose.test.yml).
...
3. Cataloging system packages from [`Dockerfile.combined`](Dockerfile.combined)
4. Identifying CI dependencies from [`.github/workflows/unit-tests.yml`](.github/workflows/unit-tests.yml), [`.github/workflows/integration-tests.yml`](.github/workflows/integration-tests.yml), and [`.github/workflows/release.yml`](.github/workflows/release.yml)
...
_Document maintained as part of the [`joplin-api`](https://github.com/werner/joplin-api) project. Update when dependencies change._
