...
The test asserts safe-behaviour checks — captured from a file on the `joplin_data` volume (written by the sync exec and read back via a helper container, since the container may die before exec stdout reaches the pipe) that currently **fail** on the buggy code and will flip to **pass** once the M2 fix lands, with zero assertion edits required.
