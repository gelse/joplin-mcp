...
The test asserts safe-behaviour checks — captured from `/home/joplin/.config/joplin/log.txt` within the same exec session as the sync (window-scoped to post-sync timestamps) — that currently **fail** on the buggy code and will flip to **pass** once the M2 fix lands, with zero assertion edits required.
